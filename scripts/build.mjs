...
const mojibakeRunPattern = /(?:\u0420.|\u0421.){3,}|(?:\u00D0.|\u00D1.){2,}|\uFFFD/u;

async function assertNoMojibakeInDist() {
  const textFiles = await collectTextFiles(dist);
  const candidates = textFiles.filter((filePath) => /\.(?:html|js|mjs|json|txt|xml|css)$/i.test(filePath));
  const hits = [];

  for (const filePath of candidates) {
    const text = await readFile(filePath, "utf8");
    if (!mojibakeRunPattern.test(text)) continue;
    hits.push(path.relative(projectRoot, filePath).replace(/\\/g, "/"));
  }

  if (hits.length) {
    throw new Error(
      `Build blocked: possible mojibake detected in generated files:\n${hits.join("\n")}`
    );
  }
}
...
await writeFile(
  ...
  injectYandexMetrika(appendJsonLdScripts(homeHtml, getTravelStructuredData())),
  "utf8"
);
...
await Promise.all([
  ...
  ...snapshotSeoPaths.map(async (pathname) => {
    ...
    await writeFile(path.join(pageDir, "index.html"), injectYandexMetrika(html), "utf8");
  }),
